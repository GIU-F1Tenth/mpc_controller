...
class TrajectoryPublisherNode(Node):
    ...
    def __init__(self):
        ...
        # Declare ROS2 parameters with config.py defaults
        self.declare_parameter('enable_logging', default_config.enable_trajectory_generation)
        ...
        try:
            with open(path, 'r') as csvfile:
                ...
                for row in reader:
                    ...
                    # (Steering angles from CSV are ignored)
                    state.delta = 0.0
                    data.append(state)
